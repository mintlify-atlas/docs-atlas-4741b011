...
<CodeGroup>
```bash POST /api/admin/categorias
...
Creates a new product category for the platform.
...
<CodeGroup>
```bash DELETE /api/admin/categorias/{id}
curl -X DELETE https://api.beanquick.com/api/admin/categorias/5 \
  -H "Authorization: Bearer {token}"
```
</CodeGroup>

Permanently deletes a category. Products using this category will need to be reassigned to another category.
